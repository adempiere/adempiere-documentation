...
Download and install a database. You can use PostgreSQL, Oracle or MySQL. PostgreSQL is recommended. Read more at [Database Server Installation & Setup](http://www.adempiere.com/Database_Server_Installation_%26_Setup).
...
* Create external tool configurations to execute programs c:\adempiere\run\_setup and c:\adempiere\run\_silentsetup as described above. Execute run\_setup the first time and then run\_silentsetup if you need to in the future. Set the parameters according to the instructions in [Application Server Installation & Setup](http://www.adempiere.com/Application_Server_Installation_%26_Setup). This will create several of the needed files: adempiere.properties and utils/myEnvironment.bat.

#### Database Setup

ADempiere needs a database to function. If you haven't already installed one, follow the instructions in [Database Server Installation & Setup](http://www.adempiere.com/Database_Server_Installation_%26_Setup).

Before you can run ADempiere, you will need to [Initialize the ADempiere Database](http://www.adempiere.com/Initialize_the_ADempiere_Database). You do this by executing the RUN\_ImportAdempiere script in the ADEMPIERE\_HOME/utils directory.
...
See [Creating WebUI Workspace using Eclipse Webtool](http://www.adempiere.com/Creating_WebUI_Workspace_using_Eclipse_Webtool).
...
If you are using a production database for testing, there is no need to import the seed data. You may have to migrate your database to a version that matches the software. You can do this by applying migration scripts \(see [Migration](http://www.adempiere.com/Migration)\) or by using a Migration Tool which will compare your database against a reference and ensure that all the necessary information is added to your database. For more information, see the [Migrate](http://www.adempiere.com/Migrate) page.
...
7. Run the RUN\_Setup script in the install location \(e.g. ${ADEMPIERE\_HOME}/Run\_Setup\). For more information see [Application Server Installation & Setup](http://www.adempiere.com/Application_Server_Installation_%26_Setup).
...
* The interesting German version [De\_DE/eclipse-setup](http://www.adempiere.com/De_DE/eclipse-setup) that goes right through debugging from the JBoss server.
* [Hacker's guide](http://www.adempiere.com/Hacker's_guide)
* [Compile](http://www.adempiere.com/Compile)
* [Creating WebUI Workspace using Eclipse Webtool](http://www.adempiere.com/Creating_WebUI_Workspace_using_Eclipse_Webtool)
